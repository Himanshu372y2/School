...
import { Users, Image as ImageIcon, Bell, BookOpen, LogOut, Plus, Trash2, Eye, EyeOff, X, GraduationCap, Edit3, MessageSquare } from 'lucide-react';
...
const AdminDashboard: React.FC = () => {
  ...
  const [activeTab, setActiveTab] = useState<'users' | 'gallery' | 'notices' | 'marks' | 'comments'>('users');
...
<button onClick={handleLogout} className="flex items-center gap-2 px-4 py-2 bg-red-500 text-white rounded-lg hover:bg-red-600 transition-colors">
        ...
        <div className="flex gap-4 mb-6">
          ...
          <button onClick={() => setActiveTab('comments')} className={`flex items-center gap-2 px-6 py-3 rounded-xl font-medium transition-all ${activeTab === 'comments' ? 'bg-blue-500 text-white shadow-lg' : 'bg-white text-gray-700 hover:bg-gray-50'}`}>
        ...
        >
          ...
          {activeTab === 'comments' && (
            <div key={`comments-section-${admin?.id}`}>
              <CommentsManagement userRole="admin" userId={admin?.id} />
            </div>
          )}
        </motion.div>
      </div>
